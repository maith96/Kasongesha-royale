  ...
  return (
    <View style={styles.wrap}>
      <View style={styles.header}>
        ...
        <Text style={ui.heading}>Campaign</Text>
      ...
      <ScrollView contentContainerStyle={styles.grid}>
        {LEVELS.map((lvl, i) => {
          ...
          return (
            ...
            >
              <Text style={styles.num}>{lvl.bonusStars !== undefined ? `Bonus ${lvl.id.slice(1)}` : `Level ${lvl.id}`}</Text>
              ...
              {open ? (
                <Text style={styles.stars}>
                  ...
                  <Text style={styles.par}>  par {lvl.par}</Text>
                </Text>
              ) : (
                <Text style={styles.lock}>🔒 {lvl.bonusStars !== undefined ? `${lvl.bonusStars} ★` : 'Finish previous'}</Text>
              )}
            </Pressable>
          );
        })}
